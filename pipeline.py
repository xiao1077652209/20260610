...
def run_output_name(filename):
    """Insert the process-wide run id before a result filename extension."""
    stem, extension = os.path.splitext(filename)
    return f"{stem}_{RUN_ID}{extension}"
...
def branch_aux_loss_enabled():
    ...
    secondary_weight = float(
        getattr(cfg, "WAVELET_AUX_LOSS_WEIGHT", 0.0)
        if modality_mode == "wavelet_multiview"
        else getattr(cfg, "IMAGE_AUX_LOSS_WEIGHT", 0.0)
    )
    spectral_weight = float(getattr(cfg, "SPECTRAL_AUX_LOSS_WEIGHT", 0.0))
    return bool(getattr(cfg, "USE_BRANCH_AUX_LOSS", False)) and (secondary_weight > 0.0 or spectral_weight > 0.0)
...
class MultiViewSpectralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.wavelet_data is not None:
            ...
            secondary = self.transform(self._to_rgb_image(image_array))
        else:
...
def validate_config():
    ...
    if cfg.EVALUATION_PROTOCOL != "strict":
        raise ValueError("EVALUATION_PROTOCOL must be 'strict'.")
    ...
    if bool(getattr(cfg, "LARGE_DATA_MODE", False)):
        if cfg.EVALUATION_PROTOCOL != "strict":
            raise ValueError("LARGE_DATA_MODE requires EVALUATION_PROTOCOL='strict'.")
        if cfg.APPLY_OVERSAMPLING:
            raise ValueError("LARGE_DATA_MODE requires APPLY_OVERSAMPLING=False.")
        print(f"  Large data mode enabled: FEATURE_DIM={cfg.FEATURE_DIM}, "
    ...
    if not 0.0 < float(getattr(cfg, "FUSION_INITIAL_SECONDARY_WEIGHT", 0.05)) < 1.0:
        raise ValueError("FUSION_INITIAL_SECONDARY_WEIGHT must be between 0 and 1.")
    ...
    if getattr(cfg, "WAVELET_AUX_LOSS_WEIGHT", 0.0) < 0.0:
        raise ValueError("WAVELET_AUX_LOSS_WEIGHT must be >= 0.0.")
...
def load_raw_dataset():
    ...
    if cfg.SPECTRAL_TARGET_LENGTH is not None and original_length != cfg.SPECTRAL_TARGET_LENGTH:
    ...
    expected_classes = getattr(cfg, "PAPER_EXPECTED_NUM_CLASSES", None)
    class_mismatch = expected_classes is not None and num_classes != int(expected_classes)
    length_mismatch = X.shape[1] != cfg.PAPER_EXPECTED_SPECTRAL_LENGTH
    if class_mismatch or length_mismatch:
        expected_description = (
            f"{int(expected_classes)} classes and {cfg.PAPER_EXPECTED_SPECTRAL_LENGTH} bands"
            if expected_classes is not None
            else f"{cfg.PAPER_EXPECTED_SPECTRAL_LENGTH} bands"
        )
        message = f"Current dataset does not match the configured expectation ({expected_description})."
    ...
    # Warn about small datasets where 4.4M-parameter model may overfit
    if len(y) < 200:
        print(f"  ⚠️  Warning: Only {len(y)} samples with {num_classes} classes. "
              f"Model has ~4.4M parameters — high overfitting risk. "
              f"Consider reducing model capacity or increasing data.")
...
def apply_paper_oversampling(X, y):
    ...
    print("=" * 70)
    strategy = str(getattr(cfg, "OVERSAMPLING_STRATEGY", "max")).lower()
...
def prepare_splits(X_raw, y_raw):
    ...
    print("=" * 70)
    # strict 协议：先划分，再预处理，最后仅对训练集过采样
    # 这是正确的做法，避免合成样本泄露到验证集和测试集
...
def build_dataloaders(splits, encoding_method, experiment_name):
    ...
    # strict 协议：仅使用训练集计算编码参数（如 RP 阈值）
...
def forward_model_outputs(model, secondary_inputs, spectra):
    outputs = model(secondary_inputs, spectra, return_dict=True)
    if not isinstance(outputs, dict):
        raise RuntimeError("Model must return a dict when return_dict=True.")
    required_keys = ("logits", "features", "secondary_features", "spectral_features")
    ...
    if branch_aux_loss_enabled():
        aux_keys = ("secondary_logits", "spectral_logits")
...
def compute_branch_auxiliary_loss(outputs, labels, criterion):
    ...
    spectral_weight = float(getattr(cfg, "SPECTRAL_AUX_LOSS_WEIGHT", 0.0))
    if secondary_weight > 0.0 and outputs["secondary_logits"] is not None:
        branch_loss = branch_loss + secondary_weight * criterion(outputs["secondary_logits"], labels)
...
def evaluate_model(model, dataloader, criterion, device, auxiliary_criterion=None, auxiliary_weight=0.0, use_auxiliary=False):
    ...
    all_labels = []
    with torch.no_grad():
        for secondary_inputs, spectra, labels in dataloader:
            secondary_inputs = secondary_inputs.to(device)
            spectra = spectra.to(device)
            labels = labels.to(device)
            outputs = forward_model_outputs(model, secondary_inputs, spectra)
...
def step_training_scheduler(scheduler, val_metrics):
    ...
    if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
        # 根据 BEST_MODEL_METRIC 动态选择监控指标
        # 与模型保存指标保持一致，避免学习率调整时机与模型保存时机不匹配
        metric = cfg.BEST_MODEL_METRIC.lower()
        if metric == "val_loss":
            scheduler.step(val_metrics["Loss"])
        else:
            # val_acc 或 val_acc_or_loss 都监控 Acc（取负值，因为 ReduceLROnPlateau 默认 mode="min"）
...
def build_model_optimizer(model):
    ...
    add_group("wavelet" if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else "image", model.secondary_branch, secondary_lr)
...
def train_model(model, dataloaders, device, experiment_name):
    ...
    if branch_aux_loss_enabled():
        secondary_label = "Wavelet" if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else "Image"
        secondary_weight = (
            getattr(cfg, "WAVELET_AUX_LOSS_WEIGHT", 0.0)
            if secondary_label == "Wavelet"
            else getattr(cfg, "IMAGE_AUX_LOSS_WEIGHT", 0.0)
        )
        print(
            f"  Branch auxiliary loss | {secondary_label} weight: {float(secondary_weight):.2f} | "
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        for secondary_inputs, spectra, labels in dataloaders["train"]:
            secondary_inputs = secondary_inputs.to(device)
            spectra = spectra.to(device)
            labels = labels.to(device)
        ...
        # 当 improved=True 时，用当前 epoch 的值直接更新 best_val_acc 和 best_val_loss
        # 修复：使用当前 epoch 的值直接赋值，而非取 max/min，避免不同 epoch 的
        # Acc 和 Loss 混合作为基准值，导致后续 should_update_best_model 判断不准确
        if improved:
...
def combine_classifier_features(fused_features, secondary_features, spectral_features):
    modality_mode = str(getattr(cfg, "MODALITY_MODE", "multimodal")).lower()
    if modality_mode == "image_only":
        return secondary_features.astype(np.float32)
    ...
    if mode == "all":
        return np.hstack([fused_features, secondary_features, spectral_features]).astype(np.float32)
    if mode == "hybrid":
        feature_blocks = [fused_features, secondary_features, spectral_features]
        if secondary_features.shape[1] == spectral_features.shape[1]:
            feature_blocks.append(np.abs(secondary_features - spectral_features))
        return np.hstack(feature_blocks).astype(np.float32)
    if mode == "enhanced":
        feature_blocks = [fused_features, secondary_features, spectral_features]
        if secondary_features.shape[1] == spectral_features.shape[1]:
            modal_mean = 0.5 * (secondary_features + spectral_features)
            feature_blocks.extend([
                np.abs(secondary_features - spectral_features),
                secondary_features * spectral_features,
            ])
...
def extract_features(model, dataloader, device):
    ...
    with torch.no_grad():
        for secondary_inputs, spectra, labels in dataloader:
            ...
            if outputs["secondary_features"] is not None:
                all_secondary_features.append(outputs["secondary_features"].cpu().numpy())
    ...
    secondary_features = np.vstack(all_secondary_features) if all_secondary_features else np.empty((len(fused_features), 0), dtype=np.float32)
    ...
    classifier_features = combine_classifier_features(fused_features, secondary_features, spectral_features)
    return {
...
def classifier_training_data(train_features, train_labels, val_features, val_labels):
    ...
    if split == "auto":
        # strict 协议下，默认使用 train+val 训练最终分类器
        split = "trainval"
    if split == "train":
...
def build_experiment_name(encoding_method, image_backbone, spectral_backbone, fusion_method, classifier_name=None):
    ...
    if modality_mode == "wavelet_multiview":
        parts = [
            ...
            str(getattr(cfg, "WAVELET_BACKBONE", "attn_cnn")).lower(),
            f"{getattr(cfg, 'WAVELET_NAME', 'db4')}_l{int(getattr(cfg, 'WAVELET_LEVEL', 3))}",
            normalize_fusion_name(fusion_method),
            cfg.EVALUATION_PROTOCOL,
        ]
        if bool(getattr(cfg, "WAVELET_INCLUDE_DENOISED", False)):
            parts.append("wdenoise")
    else:
        parts = [
    ...
    if modality_mode in {"multimodal", "image_only"} and int(getattr(cfg, "FREEZE_IMAGE_BACKBONE_STAGES", 0)) > 0:
        parts.append(f"fz{int(getattr(cfg, 'FREEZE_IMAGE_BACKBONE_STAGES', 0))}")
    image_dropout = float(getattr(cfg, "IMAGE_DROPOUT", 0.0))
    if modality_mode in {"multimodal", "image_only"} and image_dropout > 0.0:
    ...
    if normalize_fusion_name(fusion_method) == "spectral_residual":
        parts.append(f"sw{_format_float_token(getattr(cfg, 'FUSION_INITIAL_SECONDARY_WEIGHT', 0.05))}")
    ...
    if branch_aux_loss_enabled():
        modality_mode = str(getattr(cfg, "MODALITY_MODE", "multimodal")).lower()
        secondary_aux_weight = float(
            getattr(cfg, "WAVELET_AUX_LOSS_WEIGHT", 0.0)
            if modality_mode == "wavelet_multiview"
            else getattr(cfg, "IMAGE_AUX_LOSS_WEIGHT", 0.0)
        )
        spectral_aux_weight = float(getattr(cfg, "SPECTRAL_AUX_LOSS_WEIGHT", 0.0))
        if secondary_aux_weight > 0.0 and modality_mode != "spectral_only":
            prefix = "waux" if modality_mode == "wavelet_multiview" else "iaux"
            parts.append(f"{prefix}{_format_float_token(secondary_aux_weight)}")
...
def train_and_evaluate_model(encoding_method, image_backbone, spectral_backbone, fusion_method, final_classifier):
    ...
    model = MFFNINIRS(
        ...
        fusion_initial_secondary_weight=getattr(cfg, "FUSION_INITIAL_SECONDARY_WEIGHT", 0.05),
    ...
    if modality_mode == "wavelet_multiview":
        secondary_description = (
            f"wavelet ({getattr(cfg, 'WAVELET_BACKBONE', 'attn_cnn')}, "
            f"{getattr(cfg, 'WAVELET_NAME', 'db4')}, level={int(getattr(cfg, 'WAVELET_LEVEL', 3))})"
        )
    else:
        secondary_description = (
            f"image ({normalize_image_backbone_name(image_backbone)}, "
            f"dropout={float(getattr(cfg, 'IMAGE_DROPOUT', 0.0)):.2f})"
        )
    print(f"  Secondary branch: {secondary_description}")
    print(f"  Fusion dropout: {float(getattr(cfg, 'FUSION_DROPOUT', 0.0)):.2f}")

    model, history, best_epoch = train_model(model, dataloaders, device, experiment_name)
    final_secondary_weight = None
    if hasattr(model.fusion, "secondary_scale_logit"):
        final_secondary_weight = float(torch.sigmoid(model.fusion.secondary_scale_logit).detach().cpu())
        secondary_name = "wavelet" if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else "image"
        print(f"  Final residual {secondary_name} weight: {final_secondary_weight:.6f}")
    ...
    return {
        ...
        "Encoding": None if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else normalize_encoding_method(encoding_method),
        "SecondaryView": "wavelet" if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else "image",
        "ImageBackbone": None if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else normalize_image_backbone_name(image_backbone),
        "WaveletBackbone": getattr(cfg, "WAVELET_BACKBONE", None) if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else None,
        "WaveletName": getattr(cfg, "WAVELET_NAME", None) if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else None,
        "WaveletLevel": int(getattr(cfg, "WAVELET_LEVEL", 0)) if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else None,
        "WaveletChannels": (3 if bool(getattr(cfg, "WAVELET_INCLUDE_DENOISED", False)) else 2) if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else None,
        ...
        "FreezeImageBackboneStages": None if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else int(getattr(cfg, "FREEZE_IMAGE_BACKBONE_STAGES", 0)),
        "ImageDropout": None if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview" else float(getattr(cfg, "IMAGE_DROPOUT", 0.0)),
        ...
        "SecondaryBackboneLR": float(
            getattr(cfg, "WAVELET_BACKBONE_LR", cfg.LEARNING_RATE)
            if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview"
            else getattr(cfg, "IMAGE_BACKBONE_LR", cfg.LEARNING_RATE)
        ),
        "FusionHeadLR": float(getattr(cfg, "FUSION_HEAD_LR", cfg.LEARNING_RATE)),
        "InitialSecondaryWeight": float(getattr(cfg, "FUSION_INITIAL_SECONDARY_WEIGHT", 0.0)),
        "FinalSecondaryWeight": final_secondary_weight,
        ...
        "SecondaryAuxLossWeight": float(
            getattr(cfg, "WAVELET_AUX_LOSS_WEIGHT", 0.0)
            if str(getattr(cfg, "MODALITY_MODE", "")).lower() == "wavelet_multiview"
            else getattr(cfg, "IMAGE_AUX_LOSS_WEIGHT", 0.0)
        ) if branch_aux_loss_enabled() else 0.0,
...
def run_suite(experiments, summary_filename):
    ...
    output_path = os.path.join(RESULTS_DIR, run_output_name(summary_filename))
...
def run_preprocessing_benchmarks():
    ...
    output_path = os.path.join(RESULTS_DIR, run_output_name("preprocessing_benchmark.csv"))
    summary.to_csv(output_path, index=False)
...
def run_paper_benchmarks():
    ...
    if cfg.RUN_TABLE5_FUSION_AND_CLASSIFIER:
        ...
        classifier_results.to_csv(
            os.path.join(RESULTS_DIR, run_output_name("paper_table5_classifier.csv")),
            index=False,
        )
...
def run_feature_mode_benchmarks():
    ...
    output_path = os.path.join(RESULTS_DIR, run_output_name("feature_mode_benchmark.csv"))
    summary.to_csv(output_path, index=False)
...
def main():
    ...
    modality_mode = str(getattr(cfg, "MODALITY_MODE", "multimodal")).lower()
    summary_name = "spectral_wavelet" if modality_mode == "wavelet_multiview" else modality_mode
    summary.insert(0, "RunID", RUN_ID)
    summary.to_csv(
        os.path.join(RESULTS_DIR, run_output_name(f"results_summary_{summary_name}.csv")),
        index=False,
    )
